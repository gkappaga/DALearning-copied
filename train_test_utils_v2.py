...
from benchmark_analysis_v2 import ensemble_kalman_filter_analysis, bootstrap_particle_filter_analysis, stochastic_map_filter_analysis
...
def _run_stochastic_map_filter(
    ...
    H_info=None,
    plot_figures=True,
    fig_name='example_fig',
    save_pdf=False,
    infl: float = 1.0,
    loc_radius=None,
):
    import torch
    from benchmark_analysis_v2 import stochastic_map_filter_analysis
    from stochastic_maps_py.tools.metrics import lorenz_distance_matrix
    ...
    if args.dataset == "lorenz63":
        ...
        raise NotImplementedError(f"Dataset {args.dataset} not supported for SMF.")

    if H_info is None:
        H_fun, _ = mystery_operator((args.ori_dim, args.obs_dim), args.device)
    else:
        H_fun, _ = H_info

    if torch.is_tensor(args.obs_inds):
        obs_indices = args.obs_inds.tolist()
    else:
        obs_indices = list(args.obs_inds)

    default_dist = lorenz_distance_matrix(args.ori_dim) if args.dataset == "lorenz63" else torch.eye(args.ori_dim, dtype=torch.int64)
    sm_options = {
        "M": args.N,
        "distMat": torch.as_tensor(getattr(args, "sm_dist_mat", default_dist)),
        "order_all": int(getattr(args, "sm_order_all", 2)),
        "nonId_radius": int(getattr(args, "sm_nonid_radius", args.ori_dim)),
        "offdiag_rad": int(getattr(args, "sm_offdiag_radius", args.ori_dim)),
        "rho": float(getattr(args, "sm_rho", 0.0)),
    }

    rmse_list, rmv_list, rrmse_list, crps_list = [], [], [], []
    ...
    with torch.no_grad():
        for batch_ind, batch_v in enumerate(loader):
            ...
            if getattr(args, "random_noise", False):
                sigma_y_batch = torch.arange(
                    0.1,
                    1 + (0.9 / max(B - 1, 1)),
                    step=(0.9) / max(B - 1, 1),
                    device=args.device,
                ...
                sigma_y_batch = torch.full((B,), args.sigma_y, device=args.device, dtype=batch_v.dtype)

            ens_v_a = batch_v[0].unsqueeze(1).repeat(1, args.N, 1)
            ens_v_a = ens_v_a + torch.randn_like(ens_v_a, device=args.device) * args.sigma_ens
            ens_list = [ens_v_a]

            obs_y = H_fun(batch_v[0].unsqueeze(1))
            obs_y += sigma_y_batch.view(B, 1, 1) * torch.randn_like(obs_y, device=args.device)
            obs_y_list = [obs_y]
            ...
            for i in range(len(batch_v) - 1):
                ...
                for j in range(args.dt_iter):
                    if args.dataset == 'ks':
                        ...
                        ens_v_a = rk4(forward_fun, ens_v_a, i * args.dt + j * args.dt / args.dt_iter, args.dt / args.dt_iter)
                ens_v_f = ens_v_a.view(-1, args.N, args.ori_dim)
                ens_v_f = ens_v_f + torch.randn_like(ens_v_f, device=args.device) * args.sigma_v

                ens_v_a = stochastic_map_filter_analysis(
                    ensemble_f=ens_v_f,
                    observation_y=obs_y,
            ...
            observations = torch.full(batch_v.shape, float('nan'), device=args.device)
            observations[:, :, obs_indices] = obs_tensor
            ...
            rmv_tensor = torch.mean(
                torch.sqrt(args.N / max(args.N - 1, 1) * torch.mean((ens_tensor - batch_v.unsqueeze(2)) ** 2, dim=(2, 3))),
                dim=0,
            )
            crps_tensor = torch.mean(
                compute_es(ens_states=ens_tensor, true_states=batch_v, norm_p=1),
                dim=0,
            ) / torch.mean(torch.norm(batch_v, p=2, dim=2), dim=0)

            rmse_list.append(rmse_tensor)
            rmv_list.append(rmv_tensor)
            rrmse_list.append(rrmse_tensor)
            crps_list.append(crps_tensor)

            if plot_figures:
                plot_particle_trajectories_with_histograms(
                    particles=ens_tensor[:, -2, :, :],
                    true_traj=batch_v[:, -2, :],
                    observation=None,
                    dim_indices=list(range(min(4, args.ori_dim))),
    ...
    rmse_all = torch.cat(rmse_list)
    rmv_all = torch.cat(rmv_list)
    rrmse_all = torch.cat(rrmse_list)
    crps_all = torch.cat(crps_list)

    nan_mask = torch.isnan(rrmse_all)
    valid_mask = ~nan_mask

    metrics = {
        'mean_rrmse': float('nan'),
        'std_rrmse': float('nan'),
        'mean_rmse': float('nan'),
        'std_rmse': float('nan'),
        'mean_rmv': float('nan'),
        ...
        'mean_crps': float('nan'),
        'std_crps': float('nan'),
        'mean_rcrps': float('nan'),
        'std_rcrps': float('nan'),
        'mean_w2_diff': float('nan'),
        'std_w2_diff': float('nan'),
        'no_nan_percent': 0.0,
    }

    if valid_mask.any():
        metrics['mean_rrmse'], metrics['std_rrmse'] = get_mean_std(rrmse_all[valid_mask])
        metrics['mean_rmse'], metrics['std_rmse'] = get_mean_std(rmse_all[valid_mask])
        metrics['mean_rmv'], metrics['std_rmv'] = get_mean_std(rmv_all[valid_mask])
        metrics['mean_crps'], metrics['std_crps'] = get_mean_std(crps_all[valid_mask])
        metrics['no_nan_percent'] = float(valid_mask.float().mean() * 100.0)

    if plot:
        return metrics['mean_rrmse']
        ...
        for batch_ind, batch_v in enumerate(loader):
            ...
            for i in range(len(batch_v) - 1):
                ...
                if args.access_to_H:
                    ...
                    if i == 0:
                        invalid_trajs = torch.zeros(B, dtype=torch.bool, device=args.device)
                    for traj in range(B):
                        ...
                        for n in range(args.N):
                            ...
                            if args.random_h:
                                y_i = v_i @ H_matrices[traj] + eta
                            else:
                                y_i = H_fun_step(v_i.view(1, 1, -1)).view(-1) + eta
                            y_samples.append(y_i)
                        Y = torch.stack(y_samples, dim=0).squeeze(1)
                        V = v_ens
                        eta_samples = torch.stack(eta_samples, dim=0)
                        if torch.isnan(V).any() or torch.isnan(Y).any():
                            print(f"[traj {traj}] NaNs detected at time step {i+1}, skipping future steps")
                            invalid_trajs[traj] = True
                            continue  # skip calculations for this trajectory
                        # ensure 2D, enough rows, contiguous, and higher precision for the solve
                        H_tilde = torch.linalg.lstsq(V.cpu(), Y.cpu(), driver="gelsd").solution.T.to(V.device)
                        residuals = Y - (V @ H_tilde.T)
                ...
                def H_fun_tilde(v, H_tildes):
                    ...
                    Args:
                        v: Tensor of shape (B, D_state) or (B, N, D_state)
                        H_tildes: Tensor of shape (B, D_obs, D_state)

                    Returns:
                        y: Tensor of shape (B, D_obs) or (B, N, D_obs)
                    """
                    if v.ndim == 2:
                        # Single sample per batch: (B, D_state)
                        return torch.bmm(H_tildes, v.unsqueeze(-1)).squeeze(-1)  # (B, D_obs)
                    
                    elif v.ndim == 3:
                        # Ensemble input: (B, N, D_state)
                        B, N, D = v.shape
                        H_expanded = H_tildes.unsqueeze(1).expand(B, N, *H_tildes.shape[1:])  # (B, N, D_obs, D_state)
                        v = v.unsqueeze(-1)  # (B, N, D_state, 1)
                        y = torch.matmul(H_expanded, v).squeeze(-1)  # (B, N, D_obs)
                        return y
                    
                    else:
                        raise ValueError(f"Input v must be of shape (B, D_state) or (B, N, D_state), got {v.shape}")
                if not args.access_to_H and not args.access_to_noise:
                    valid_mask = ~invalid_trajs
                    H_tildes_valid = H_tildes[valid_mask]   # (B_valid, d_obs, d_state)

                    def H_fun_approx(V):
                        return H_fun_tilde(V, H_tildes_valid)
                # print(H_tildes)
                common_enkf_args = {
                    ...
                    "invalid_trajs": invalid_trajs if not args.access_to_H else None,
                }

                if not args.access_to_H:
                    common_enkf_args['observation_operator_ens'] = H_fun_approx
                if not args.access_to_noise:
                    common_enkf_args['Gamma_Tilde'] = Gammas
                if args.random_noise:
                    common_enkf_args['sigma_y'] = sigma_y_batch
                ...
                if args.v == 'EnKF':
                    ...
                    # Support batch-varying sigma_y if your code already provides it
                    if 'sigma_y_batch' in locals() and sigma_y_batch is not None:
                        sy = sigma_y_batch
                    else:
                        sy = args.sigma_y  # scalar or tensor

                    # Call SMF on the forecast ensemble; capture either (X_a, transform) or X_a
                    smf_out = stochastic_map_filter_analysis(
                        particles_forecast=ens_v_f,                  # (B, N, d)
                        observation_y=obs_y_b,                       # (B, m)
                        observation_operator=obs_op,                 # H or callable h(x)
                        sigma_y=sy,
                        # --- repo-style options ---
                        M=ens_v_f.shape[1],                          # number of particles (optional)
                        distMat=getattr(args, 'smf_distMat', None),  # (d, d) tensor or None
                        order_all=getattr(args, 'smf_order', 1),
                        nonId_radius=getattr(args, 'smf_nonId_radius', None),
                    ...
                    if isinstance(smf_out, tuple) and len(smf_out) == 2:
                        ens_v_a, kalman_gain_or_transform = smf_out
                    else:
                        ens_v_a = smf_out
                        kalman_gain_or_transform = None  # or a small placeholder object if your code expects one

                    # (Optional) Debug/logging hook if you want to record the transform type
                    if kalman_gain_or_transform is not None and hasattr(kalman_gain_or_transform, 'kind'):
                        # Example: print or stash metadata for later analysis
                        _meta = getattr(kalman_gain_or_transform, 'meta', {})
                        # logger.info(f"SMF transform: {kalman_gain_or_transform.kind}, meta={_meta}")
                ...
                ens_v_a = torch.clamp(ens_v_a, min=-args.clamp, max=args.clamp)
                ens_list.append(ens_v_a)
                # print(torch.stack(ens_list).shape)
                # print(torch.stack(ens_list).shape)
            ...
            if args.access_to_H:
                ...
                ens_tensor = torch.full(
                    (len(batch_v), B, args.N, args.ori_dim),
                    float('nan'),
                    device=args.device
                )
                for traj in range(B):
                    if invalid_trajs[traj]:
                        continue  # keep NaNs for this trajectory
                    stacked = torch.stack(ens_list)
                    ens_tensor[:, traj] = stacked[:, traj]
            if args.access_to_H:
                crps_val = torch.mean(compute_es(ens_states=ens_tensor, true_states=batch_v, norm_p=1), dim=0)
                rcrps_val = crps_val / torch.mean(torch.norm(batch_v, p=2, dim=2), dim=0)
                rmse_val = torch.mean(torch.sqrt(torch.mean((ens_tensor.mean(dim=2) - batch_v) ** 2, dim=2)), dim=0)
                ...
                # T = ens_tensor.shape[0]
                # B = ens_tensor.shape[1]
                # N_ens = ens_tensor.shape[2]
                # scale = N_ens / (N_ens - 1)

                # rmv_acc = torch.zeros(B, device=ens_tensor.device, dtype=ens_tensor.dtype)

                # for t in range(T):
                #     # [B, N_ens, D] minus [B, 1, D] -> [B, N_ens, D]
                #     diff = ens_tensor[t] - batch_v[t].unsqueeze(1)
                #     mse  = diff.square().mean(dim=(1, 2))          # [B]
                #     rmv_acc += torch.sqrt(scale * mse)             # [B]

                # rmv_val = rmv_acc / T

                # T = len(ens_list)
                # B = batch_v.shape[1]
                # device = batch_v.device
                # dtype = batch_v.dtype

                # rmse_acc = torch.zeros(B, device=device, dtype=dtype)
                # rmv_acc = torch.zeros(B, device=device, dtype=dtype)
                # crps_acc = torch.zeros(B, device=device, dtype=dtype)
                # rms_acc = torch.zeros(B, device=device, dtype=dtype)
                # true_norm_acc = torch.zeros(B, device=device, dtype=dtype)

                # scale = args.N / max(args.N - 1, 1)
                ...
                #     rmse_acc += torch.sqrt(torch.mean((mean_t - true_t) ** 2, dim=1))

                #     # RMV contribution
                #     diff_t = ens_t - true_t.unsqueeze(1)  # [B, N, D]
                #     rmv_acc += torch.sqrt(scale * torch.mean(diff_t ** 2, dim=(1, 2)))

                #     # CRPS / ES contribution: one timestep only
                #     crps_acc += compute_es(
                ...
                #     # denominators
                #     rms_acc += torch.sqrt(torch.mean(true_t ** 2, dim=1))
                #     true_norm_acc += torch.norm(true_t, p=2, dim=1)
                ...
                # rms_val = rms_acc / T
                # rrmse_val = rmse_val / rms_val
                # rcrps_val = crps_val / (true_norm_acc / T)
                ...
                valid_mask = ~invalid_trajs
                ens_tensor_valid = ens_tensor[:, valid_mask]
                batch_v_valid    = batch_v[:, valid_mask]

                # Step 3: Compute metrics ONLY on valid trajs
                crps_val = torch.mean(compute_es(
                    ens_states=ens_tensor_valid, true_states=batch_v_valid, norm_p=1
                ), dim=0)

                rcrps_val = crps_val / torch.mean(torch.norm(batch_v_valid, p=2, dim=2), dim=0)

                rmse_val = torch.mean(torch.sqrt(torch.mean(
                    (ens_tensor_valid.mean(dim=2) - batch_v_valid) ** 2, dim=2
                )), dim=0)

                rms_val = torch.mean(torch.sqrt(torch.mean(batch_v_valid ** 2, dim=2)), dim=0)
                rrmse_val = rmse_val / rms_val

                rmv_val = torch.mean(torch.sqrt(
                    N_ens / (N_ens-1) *
                    torch.mean((ens_tensor_valid - batch_v_valid.unsqueeze(2)) ** 2, dim=(2, 3))
                ), dim=0)

                # Step 4: Scatter back into (B,) with NaNs for invalid trajs
                def scatter_to_full(valid_vals, B, valid_mask, device):
                    out = torch.full((B,), float('nan'), device=device)
                    out[valid_mask] = valid_vals
                    return out

                crps_val  = scatter_to_full(crps_val,  B, valid_mask, batch_v.device)
                rcrps_val = scatter_to_full(rcrps_val, B, valid_mask, batch_v.device)
                rmse_val  = scatter_to_full(rmse_val,  B, valid_mask, batch_v.device)
                rrmse_val = scatter_to_full(rrmse_val, B, valid_mask, batch_v.device)
                rmv_val   = scatter_to_full(rmv_val,   B, valid_mask, batch_v.device)
